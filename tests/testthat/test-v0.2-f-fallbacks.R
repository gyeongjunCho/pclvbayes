test_that("canonical CV spline failures are explicit and never substituted", {
  ...
  expect_error(
    pclvbayes:::.smooth_spline_robust(
      1:4,
      1:4,
      spline_df = 3
    ),
...
test_that("invalid time intervals are rejected rather than repaired", {
  duplicate <- pclvbayes:::.delta_alr_over_dt(
    c(0, 1, 2),
    c(0, 1, 1),
    rep("A", 3)
  )

  nonfinite <- pclvbayes:::.delta_alr_over_dt(
    c(0, 1),
    c(0, Inf),
    rep("A", 2)
  )
...
test_that("missing OU persistence and invalid nu cannot select another scorer", {
  ...
  # All other required predictive draws are valid, so absence of both
  # lambda and phi must specifically fail at the OU-persistence check.
  ...
  expect_identical(
    missing$reason,
    "missing_ou_persistence_draws"
  )

  # Supply a valid persistence parameter and make only nu invalid.
  base$phi <- .8
  base$nu <- NA_real_
...
test_that("malformed initialization stops and retry exhaustion exposes provenance", {
  ...
  expect_equal(
    vapply(out$retry_history, `[[`, integer(1), "attempt"),
    1:2
  )

  expect_identical(
    out$failure$details$attempt_history,
    out$retry_history
  )

  expect_equal(
    vapply(out$retry_history, `[[`, numeric(1), "seed"),
    c(9, 10)
  )
