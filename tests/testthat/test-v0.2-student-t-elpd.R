test_that("Student-t Kalman scoring reduces exactly to Student-t without latent-state uncertainty", {
  ...
  expect_identical(
    scored$state_approximation,
    "gaussian-moment-collapse"
  )
})
...
test_that("Student-t scale-mixture Kalman scoring is deterministic", {
  ...
  first <- pclvbayes:::.proj_loglik_subject(
    draws,
    held_out
  )

  second <- pclvbayes:::.proj_loglik_subject(
    draws,
    held_out
  )

  expect_false(inherits(first, "pclv_failure"))
  expect_false(inherits(second, "pclv_failure"))
...
test_that("new-subject predictive variance integrates random-intercept uncertainty", {
  draws <- data.frame(
    r0 = 0,
    a_ii = 0,
    a_ij = 0,
...
test_that("accepted Student-t scale audit is chain-specific and rejects accepted zeros", {
  ...
  audit <- pclvbayes:::.accepted_scale_audit(
    draws
  )
